var resources_git = [
  /*
    {
        titulo: "Plantilla",
        url: "http://www.ejemplo.com",
        tags: ["init", "clone","ramas", "local", "remoto"],
        temas: ["Git", "Trello"],
        formatos: ["pdf"],
        nivel: ["básico", "medio", "avanzado"]  // elegir solo uno
    },
    */
  {
    titulo: "Sintaxis Markdown",
    url: "https://markdown.es/sintaxis-markdown/",
    tags: ["markdown"],
    temas: ["Git"],
    formatos: ["web", "vídeo"],
    nivel: ["básico"],
    idiomas: ["español"],
  },
  {
    titulo: "Cómo instalar GIT en Windows, MacOS y Linux.",
    url: "https://www.hostinger.es/tutoriales/instalar-git-en-distintos-sistemas-operativos",
    tags: ["init"],
    temas: ["Git"],
    formatos: ["web"],
    nivel: ["básico"],
    idiomas: ["español"],
  },
  {
    titulo: "Conociendo Git y creando mi primer repositorio local.",
    url: "https://es.linkedin.com/pulse/conociendo-git-y-creando-mi-primer-repositorio-local-alarc%C3%B3n-acurero",
    tags: ["git", "init", "local"],
    temas: ["Git"],
    formatos: ["web"],
    nivel: ["básico"],
    idiomas: ["español"],
  },
  {
    titulo: "Navegando por el Terminal Git Bash.",
    url: "https://es.linkedin.com/pulse/navengando-por-el-terminal-git-bash-para-alarc%C3%B3n-acurero?trk=public_profile_article_view",
    tags: ["init"],
    temas: ["Git"],
    formatos: ["web"],
    nivel: ["básico"],
    idiomas: ["español"],
  },
  {
    titulo: "Buenas prácticas al trabajar con Git.",
    url: "https://david-estevez.gitbooks.io/the-git-the-bad-and-the-ugly/content/es/buenas-temass-al-trabajar-con-git.html",
    tags: ["ramas"],
    temas: ["Git"],
    formatos: ["web"],
    nivel: ["medio"],
    idiomas: ["español"],
  },
  {
    titulo: "Trabajar con ramas en Git.",
    url: "https://desarrolloweb.com/articulos/trabajar-ramas-git.html",
    tags: ["ramas", "remoto"],
    temas: ["Git"],
    formatos: ["web"],
    nivel: ["básico"],
    idiomas: ["español"],
  },
  {
    titulo: "Cómo fusionar ramas con git merge y resolver conflictos.",
    url: "https://www.atlassian.com/es/git/tutorials/using-branches/git-merge",
    tags: ["ramas", "local"],
    temas: ["Git"],
    formatos: ["web"],
    nivel: ["medio"],
    idiomas: ["español"],
  },
  {
    titulo:
      "Cómo actualizar un repositorio local y remoto con git push y git pull.",
    url: "https://leninner.medium.com/gu%C3%ADa-sencilla-para-trabajar-con-git-push-y-git-pull-785e0bca60cc",
    tags: ["local", "remoto"],
    temas: ["Git"],
    formatos: ["web"],
    nivel: ["medio"],
    idiomas: ["español"],
  },
  {
    titulo: "¿Para qué sirven las etiquetas (tags) en git?",
    url: "https://www.youtube.com/watch?v=ywzdb2brvt8",
    tags: ["local", "remoto"],
    temas: ["Git"],
    formatos: ["vídeo"],
    nivel: ["medio"],
    duracion: 4,
    idiomas: ["español"],
  },
  {
    titulo: "Listado de comandos en git.",
    url: "https://gist.github.com/dasdo/9ff71c5c0efa037441b6",
    tags: ["local"],
    temas: ["Git"],
    formatos: ["web"],
    nivel: ["avanzado"],
    idiomas: ["español"],
  },
  {
    titulo: "¿Qué son las GitHub Actions?",
    url: "https://www.youtube.com/shorts/3bEhblIDXFY",
    tags: ["remoto"],
    temas: ["Git"],
    formatos: ["vídeo"],
    nivel: ["avanzado"],
    duracion: 1,
    idiomas: ["español"],
  },
  {
    titulo: "¿Cómo excluir archivos del control de versiones? (.gitignore)",
    url: "https://www.youtube.com/watch?v=w44lFiaiAiw",
    tags: ["local", "remoto"],
    temas: ["Git"],
    formatos: ["vídeo"],
    nivel: ["medio"],
    duracion: 4,
    idiomas: ["español"],
  },
  {
    titulo: "Crear repositorio remoto en GitHub.",
    url: "https://docs.github.com/es/repositories/creating-and-managing-repositories/quickstart-for-repositories",
    tags: ["remoto"],
    temas: ["Git"],
    formatos: ["web"],
    nivel: ["básico"],
    idiomas: ["español"],
  },
  {
    titulo: "¿Qué es el lenguaje de marcado Markdown?",
    url: "https://www.youtube.com/shorts/6yn1cSko-8Q",
    tags: ["markdown"],
    temas: ["Git"],
    formatos: ["vídeo"],
    nivel: ["básico"],
    duracion: 1,
    idiomas: ["español"],
  },
  {
    titulo: "StackEdit: In-browser Markdown editor",
    url: "https://stackedit.io/",
    tags: ["markdown"],
    temas: ["Git"],
    formatos: ["web"],
    nivel: ["básico"],
    idiomas: ["inglés"],
  },
  {
    titulo: "Markdown Guide. Basic Syntax.",
    url: "https://www.markdownguide.org/basic-syntax/",
    tags: ["markdown"],
    temas: ["Git"],
    formatos: ["web"],
    nivel: ["avanzado"],
    idiomas: ["inglés"],
  },
  {
    titulo:
      "Cómo contribuir en un repositorio en GitHub con Fork y Pull Request(PR).",
    url: "https://www.freecodecamp.org/espanol/news/como-hacer-tu-primer-pull-request-en-github/",
    tags: ["local", "ramas", "remoto"],
    temas: ["Git"],
    formatos: ["web"],
    nivel: ["avanzado"],
    idiomas: ["español"],
  },
  {
    titulo: "Crear y asignar propuestas (issues) en un repositorio de GitHub.",
    url: "https://docs.github.com/es/issues/tracking-your-work-with-issues/quickstart",
    tags: ["remoto"],
    temas: ["Git"],
    formatos: ["web"],
    nivel: ["medio"],
    idiomas: ["español"],
  },
  {
    titulo: "Deshacer cambios en Git con git revert.",
    url: "https://www.youtube.com/watch?v=4SwaV29SpIc",
    tags: ["local"],
    temas: ["Git"],
    formatos: ["vídeo"],
    nivel: ["medio"],
    duracion: 7,
    idiomas: ["español"],
  },
  {
    titulo: "Estados en Git.",
    url: "https://medium.com/laboratoria-how-to/describiendo-el-flujo-de-trabajo-en-git-ede2eee5b589",
    tags: ["local"],
    temas: ["Git"],
    formatos: ["web"],
    nivel: ["básico"],
    idiomas: ["español"],
  },
  {
    titulo: "Learn Git Rebase.",
    url: "https://www.youtube.com/watch?v=f1wnYdLEpgI",
    tags: ["local"],
    temas: ["Git"],
    formatos: ["vídeo"],
    nivel: ["avanzado"],
    duracion: 7,
    idiomas: ["inglés"],
  },
  {
    titulo: "Cómo descargar carpetas y ficheros de Github.",
    url: "https://www.youtube.com/watch?v=KzZcnBzI3OU",
    tags: ["remoto"],
    temas: ["Git"],
    formatos: ["vídeo"],
    nivel: ["básico"],
    duracion: 2,
    idiomas: ["español"],
  },
  {
    titulo: "Configura Gitignore para cualquier tecnología.",
    url: "https://www.youtube.com/shorts/Vh3PKXFuaFs",
    tags: ["local", "remoto"],
    temas: ["Git"],
    formatos: ["vídeo"],
    nivel: ["medio"],
    duracion: 1,
    idiomas: ["español"],
  },
  {
    titulo: "Cómo clonar un repositorio de github.",
    url: "https://www.youtube.com/shorts/KvecZDvMQpc",
    tags: ["clone"],
    temas: ["Git"],
    formatos: ["vídeo"],
    nivel: ["básico"],
    duracion: 1,
    idiomas: ["español"],
  },
  {
    titulo: "What’s the difference between merging and rebasing in Git?",
    url: "https://www.youtube.com/shorts/nzv0sbfprJo",
    tags: ["local"],
    temas: ["Git"],
    formatos: ["vídeo"],
    nivel: ["avanzado"],
    duracion: 1,
    idiomas: ["inglés"],
  },
  {
    titulo: "Qué es y cómo usar git stash.",
    url: "https://www.youtube.com/watch?v=fD7ZOK5kZMs",
    tags: ["local"],
    temas: ["Git"],
    formatos: ["vídeo"],
    nivel: ["avanzado"],
    duracion: 6,
    idiomas: ["español"],
  },
  {
    titulo: "Actualizar un commit en Git.",
    url: "https://www.youtube.com/shorts/ku2n0mKzkPk",
    tags: ["local"],
    temas: ["Git"],
    formatos: ["vídeo"],
    nivel: ["básico"],
    duracion: 1,
    idiomas: ["español"],
  },
  {
    titulo: "Curso de git y github desde cero para principiantes.",
    url: "https://www.youtube.com/watch?v=3GymExBkKjE",
    tags: ["clone", "init", "local", "markdown", "ramas", "remoto"],
    temas: ["Git"],
    formatos: ["vídeo"],
    nivel: ["básico"],
    duracion: 305,
    idiomas: ["español"],
  },
  {
    titulo: "Git Succinctly ebook.",
    url: "https://www.syncfusion.com/succinctly-free-ebooks/git",
    tags: ["clone", "init", "local", "ramas", "remoto"],
    temas: ["Git"],
    formatos: ["pdf"],
    nivel: ["avanzado"],
    idiomas: ["inglés"],
  },

  {
    titulo: "Managing Git Repositories with IntelliJ.",
    url: "https://www.jetbrains.com/help/idea/using-git-integration.html",
    tags: ["init", "local", "clone", "ramas", "remoto"],
    temas: ["Git"],
    formatos: ["web"],
    nivel: ["medio"],
    idiomas: ["inglés"],
  },
  {
    titulo: "Managing GitHub Repositories with IntelliJ.",
    url: "https://www.jetbrains.com/help/idea/github.html",
    tags: ["clone", "ramas", "remoto"],
    temas: ["Git"],
    formatos: ["web"],
    nivel: ["medio"],
    idiomas: ["inglés"],
  },
  {
    titulo: "Managing Git Repositories with Clion.",
    url: "https://www.jetbrains.com/help/clion/using-git-integration.html",
    tags: ["init", "local", "clone", "ramas", "remoto"],
    temas: ["Git"],
    formatos: ["web"],
    nivel: ["medio"],
    idiomas: ["inglés"],
  },
  {
    titulo: "Managing GitHub Repositories with Clion.",
    url: "https://www.jetbrains.com/help/clion/github.html",
    tags: ["clone", "ramas", "remoto"],
    temas: ["Git"],
    formatos: ["web"],
    nivel: ["medio"],
    idiomas: ["inglés"],
  },
  {
    titulo: "ProGit ebook. (Español)",
    url: "https://git-scm.com/book/es/v2",
    tags: ["init", "local", "clone", "ramas", "remoto"],
    temas: ["Git"],
    formatos: ["pdf"],
    nivel: ["avanzado"],
    idiomas: ["español"],
  },
  {
    titulo: "ProGit ebook. (English)",
    url: "https://git-scm.com/book/en/v2",
    tags: ["init", "local", "clone", "ramas", "remoto"],
    temas: ["Git"],
    formatos: ["pdf"],
    nivel: ["avanzado"],
    idiomas: ["inglés"],
  },
  {
    titulo: "Git by example: Interactive guide.",
    url: "https://antonz.org/git-by-example/",
    tags: ["init", "local", "clone", "ramas", "remoto"],
    temas: ["Git"],
    formatos: ["web"],
    nivel: ["avanzado"],
    idiomas: ["inglés"],
  },
  {
    titulo: "Hoja de comandos Git-GitHub.",
    url: "https://training.github.com/downloads/es_ES/github-git-cheat-sheet.pdf",
    tags: ["init", "local", "clone", "ramas", "remoto"],
    temas: ["Git"],
    formatos: ["pdf"],
    nivel: ["avanzado"],
    idiomas: ["español"],
  },
  {
    titulo: "Git for beginners: The definitive temasl guide.",
    url: "https://stackoverflow.com/questions/315911/git-for-beginners-the-definitive-temasl-guide",
    tags: ["init", "local", "clone", "ramas", "remoto"],
    temas: ["Git"],
    formatos: ["web"],
    nivel: ["avanzado"],
    idiomas: ["inglés"],
  },
  {
    titulo: "An open source game about learning Git!",
    url: "https://ohmygit.org/",
    tags: ["init", "local", "ramas"],
    temas: ["Git"],
    formatos: ["web"],
    nivel: ["básico"],
    idiomas: ["inglés"],
  },
  {
    titulo: "Learn Git Branching!",
    url: "https://learngitbranching.js.org/?locale=es_ES",
    tags: ["init", "local", "clone", "ramas", "remoto"],
    temas: ["Git"],
    formatos: ["web"],
    nivel: ["medio"],
    idiomas: ["español"],
  },
    {
    titulo: "The Markdown Guide",
    url: "https://www.markdownguide.org/",
    tags: ["markdown"],
    temas: ["Git"],
    formatos: ["web"],
    nivel: ["medio"],
    idiomas: ["inglés"],
  },
];
